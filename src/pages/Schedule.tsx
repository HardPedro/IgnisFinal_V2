  ...
  return (
    <div className="max-w-7xl mx-auto">
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-bold text-blue-950 tracking-tight">Agenda</h1>
          <p className="text-slate-500 mt-1">Gerencie seus compromissos e agendamentos.</p>
        </div>
        <button
          onClick={() => { resetForm(); setIsModalOpen(true); }}
          className="bg-orange-500 text-white px-4 py-2 rounded-xl font-bold hover:bg-orange-600 transition-colors flex items-center shadow-sm"
        >
      ...
      <div className="bg-white rounded-[32px] shadow-sm border border-slate-200 overflow-hidden">
        <div className="p-6 border-b border-slate-200 flex items-center justify-between bg-slate-50/50">
          <div className="flex items-center space-x-4">
            <button onClick={prevWeek} className="p-2 hover:bg-white rounded-lg transition-colors border border-transparent hover:border-slate-200 hover:shadow-sm text-slate-600">
              &lt;
            </button>
            <h2 className="text-lg font-bold text-blue-950 capitalize w-48 text-center">
              {format(startDate, 'MMMM yyyy', { locale: ptBR })}
            </h2>
            <button onClick={nextWeek} className="p-2 hover:bg-white rounded-lg transition-colors border border-transparent hover:border-slate-200 hover:shadow-sm text-slate-600">
              &gt;
            </button>
          </div>
          <button onClick={today} className="px-4 py-2 text-sm font-medium text-slate-700 bg-white border border-slate-200 rounded-xl hover:bg-slate-50 transition-colors shadow-sm">
            Hoje
          </button>
        </div>
        ...
        <div className="grid grid-cols-7 border-b border-slate-200">
          {weekDays.map((day, i) => (
            <div key={i} className={`p-4 text-center border-r border-slate-200 last:border-r-0 ${isSameDay(day, new Date()) ? 'bg-orange-50/30' : ''}`}>
              <div className="text-xs font-medium text-slate-500 uppercase mb-1">{format(day, 'EEE', { locale: ptBR })}</div>
              <div className={`text-lg font-bold ${isSameDay(day, new Date()) ? 'text-orange-600' : 'text-blue-950'}`}>
                {format(day, 'd')}
              </div>
            </div>
          ))}
        ...
        <div className="grid grid-cols-7 min-h-[500px]">
          {weekDays.map((day, i) => {
            ...
            return (
              <div key={i} className={`p-2 border-r border-slate-200 last:border-r-0 ${isSameDay(day, new Date()) ? 'bg-orange-50/10' : ''}`}>
                <div className="space-y-2">
                  {dayApps.map(app => (
                    ...
                    >
                      ...
                      {app.customerId && (
                        <div className="mt-2 text-xs text-slate-500 flex items-center truncate">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full border border-slate-300 rounded-xl px-4 py-2 focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-colors"
              ...
              <div className="pt-4 flex justify-between space-x-3 border-t border-slate-100 mt-6">
                {editingId ? (
                  <button
                    type="button"
                    onClick={() => handleDelete(editingId)}
                    className="px-4 py-2 text-red-600 bg-red-50 hover:bg-red-100 rounded-xl font-medium transition-colors mt-4"
                  >
                    Excluir
                  </button>
