...
export function Vehicles() {
  ...
  const handleDelete = async (id: string) => {
    ...
    if (window.confirm('Tem certeza que deseja excluir este veículo?')) {
      try {
        await deleteDoc(doc(db, `tenants/${userData.tenantId}/vehicles`, id));
      } catch (err) {
    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-bold text-blue-950 tracking-tight flex items-center">
          <Car className="mr-3 h-8 w-8 text-orange-500" />
          Veículos
        </h1>
        <button
        ...
        {isAdding && (
          ...
          >
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 mb-6">
              <h2 className="text-lg font-medium text-blue-950 mb-4">Adicionar Veículo</h2>
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="block w-full rounded-xl border-slate-300 shadow-sm focus:border-orange-500 focus:ring-orange-500 sm:text-sm px-4 py-3 border uppercase transition-colors"
      ...
      <div className="bg-white shadow-sm overflow-hidden sm:rounded-2xl border border-slate-200">
        <ul className="divide-y divide-slate-100">
          {isLoading ? (
            ...
            <li className="p-8 text-center text-slate-500">Nenhum veículo encontrado.</li>
          ) : (
            vehicles.map((vehicle) => (
              <li key={vehicle.id} className="px-6 py-5 flex items-center justify-between hover:bg-slate-50 transition-colors">
                <div className="flex items-center">
                  ...
                  <div>
                    <p className="text-sm font-medium text-blue-950">{vehicle.make} {vehicle.model} {vehicle.year}</p>
                    <p className="text-sm text-slate-500">Placa: <span className="font-mono bg-slate-100 px-1 rounded">{vehicle.plate || 'N/A'}</span> • Cliente: {vehicle.customerName}</p>
                  </div>
                </div>
