...
export function Customers() {
  ...
  useEffect(() => {
    ...
    const workOrdersRef = collection(db, `tenants/${tenantId}/work_orders`);
    ...
    const unsubWorkOrders = onSnapshot(
      workOrdersRef,
      (snapshot) => {
        workOrdersData = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
        calculateRecalls();
      },
      (error) => handleFirestoreError(error, OperationType.LIST, `tenants/${tenantId}/work_orders`, currentUser)
    );
  ...
  const handleDelete = async (id: string) => {
    ...
    if (window.confirm('Tem certeza que deseja excluir este cliente?')) {
      try {
        await deleteDoc(doc(db, `tenants/${userData.tenantId}/customers`, id));
      } catch (err) {
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-6">
          <h1 className="text-3xl font-bold text-blue-950 tracking-tight flex items-center">
            <Users className="mr-3 h-8 w-8 text-orange-500" />
            Clientes
          </h1>
          <div className="flex space-x-2 bg-slate-100 p-1 rounded-xl">
            <button
              onClick={() => setActiveTab('list')}
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${activeTab === 'list' ? 'bg-white text-blue-950 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
            >
              Lista
            </button>
            <button
              onClick={() => setActiveTab('recalls')}
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors flex items-center ${activeTab === 'recalls' ? 'bg-white text-blue-950 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
            >
              Recalls
              {recalls.length > 0 && (
        ...
        {isAdding && (
          ...
          >
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 mb-6">
              <h2 className="text-lg font-medium text-blue-950 mb-4">Adicionar Cliente</h2>
              <form onSubmit={handleAdd} className="space-y-4">
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <div>
                    ...
                    <input
                      ...
                      className="block w-full rounded-xl border-slate-300 shadow-sm focus:border-orange-500 focus:ring-orange-500 sm:text-sm px-4 py-3 border transition-colors"
                ...
                <div className="flex justify-end space-x-3 pt-4">
                  <button
                    ...
                    className="px-4 py-2 border border-slate-300 shadow-sm text-sm font-medium rounded-xl text-slate-700 bg-white hover:bg-slate-50 transition-colors"
                  >
                    Cancelar
                  </button>
                  <button
                    type="submit"
                    className="px-4 py-2 border border-transparent shadow-sm text-sm font-bold rounded-xl text-white bg-orange-500 hover:bg-orange-600 transition-colors"
                  >
                    Salvar Cliente
                  </button>
                </div>
        ...
        <div className="bg-white shadow-sm overflow-hidden sm:rounded-2xl border border-slate-200">
          <ul className="divide-y divide-slate-100">
            {isLoading ? (
              ...
              <li className="p-8 text-center text-slate-500">Nenhum cliente encontrado.</li>
            ) : (
              customers.map((customer) => (
                <li key={customer.id} className="px-6 py-5 flex items-center justify-between hover:bg-slate-50 transition-colors">
                  <div className="flex items-center">
                    <div className="h-10 w-10 rounded-full bg-orange-100 flex items-center justify-center text-orange-700 font-bold mr-4">
                      {customer.name.charAt(0).toUpperCase()}
                    </div>
                    <div>
                      <p className="text-sm font-medium text-blue-950">{customer.name}</p>
        ...
        <div className="bg-white shadow-sm overflow-hidden sm:rounded-2xl border border-slate-200">
          <div className="px-6 py-5 border-b border-slate-200 bg-white flex items-center justify-between">
            <div className="flex items-center">
              ...
              <div>
                <h3 className="text-lg leading-6 font-semibold text-blue-950">Recalls Pendentes</h3>
                <p className="text-sm text-slate-500">Clientes que precisam retornar para serviços recorrentes</p>
              </div>
            </div>
          </div>
          <ul className="divide-y divide-slate-100">
            {isLoading ? (
              ...
              <li className="p-8 text-center text-slate-500">Nenhum recall pendente no momento.</li>
            ) : (
              recalls.map((recall, idx) => {
                ...
                return (
                  <li key={idx} className="px-6 py-5 flex items-center justify-between hover:bg-slate-50 transition-colors">
                    <div className="flex items-center">
                      <div className={`h-10 w-10 rounded-full flex items-center justify-center font-bold mr-4 ${isOverdue ? 'bg-red-100 text-red-700' : 'bg-orange-100 text-orange-700'}`}>
                        {isOverdue ? <AlertCircle className="h-5 w-5" /> : <Clock className="h-5 w-5" />}
                      </div>
                      <div>
                        <p className="text-sm font-medium text-blue-950">{recall.customer_name} <span className="text-slate-500 font-normal">- {recall.customer_phone}</span></p>
                        <p className="text-sm text-slate-600 mt-1">
                          <span className="font-medium text-blue-950">{recall.service_name}</span> no veículo <span className="font-mono bg-slate-100 px-1 rounded">{recall.vehicle_plate}</span>
                        </p>
                      </div>
                    </div>
                    <div className="text-right">
                      <div className={`text-sm font-bold px-3 py-1 rounded-lg inline-block ${isOverdue ? 'bg-red-50 text-red-700' : 'bg-orange-50 text-orange-700'}`}>
                        {isOverdue ? 'Atrasado' : 'Vence em'} {new Date(recall.next_service_date).toLocaleDateString('pt-BR')}
                      </div>
                      <p className="text-xs text-slate-400 mt-1">Última OS: {new Date(recall.last_service_date).toLocaleDateString('pt-BR')}</p>
                    </div>
                  </li>
                );
              })
